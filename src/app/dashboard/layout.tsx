...
export default async function DashboardLayout({
  ...
  // Supabase relationship selects can return either an object or an array depending on FK/cardinality.
  // Normalize to the shape expected by `DashboardTopbar`.
  const normalizedPlanAssignments = (planAssignments ?? []).map((pa: any) => ({
    id: String(pa.id),
    plans: Array.isArray(pa.plans) ? pa.plans[0] ?? null : pa.plans ?? null,
    organizations: Array.isArray(pa.organizations) ? pa.organizations[0] ?? null : pa.organizations ?? null,
  }));
