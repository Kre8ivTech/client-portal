    ...
    <div className="w-full space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              {isStaff && <DeliverableForm ticketId={ticket.id} onSuccess={refreshDeliverables} />}
            </div>
        ...
        {isStaff && organizationId && (
          <TicketTimeTracking ticketId={ticket.id} organizationId={organizationId} userId={userId} isStaff={isStaff} />
        )}
        ...
        <div className="space-y-6">
          <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm space-y-6">
            ...
            <DetailItem
              ...
              label="Time Logged"
              value={formatTimeLogged((ticket as any).total_time_logged)}
            />
            ...
            {canAssign ? (
              <div className="pt-2 border-t mt-4">
                <span className="text-xs font-semibold text-slate-400 uppercase tracking-tighter mb-2 block">
                  Assigned To
                </span>
                <Select
...
function StatusBadge({ status }: { status: Ticket["status"] }) {
  return (
    <Badge
      className={`${STATUS_STYLES[status] ?? STATUS_STYLES.new} font-bold capitalize px-2.5 py-0.5 rounded-full border shadow-sm text-[11px]`}
    >
      {status.replace("_", " ")}
    </Badge>
  );
...
function PriorityBadge({ priority }: { priority: Ticket["priority"] }) {
  return (
    <Badge
      variant="outline"
      className={`${PRIORITY_STYLES[priority] ?? PRIORITY_STYLES.medium} font-bold capitalize px-2.5 py-0.5 rounded-full text-[11px] shadow-sm`}
    >
      {priority}
    </Badge>
  );
...
function formatTimeLogged(hours: number | null | undefined): string {
  if (!hours || hours === 0) return "No time logged";
