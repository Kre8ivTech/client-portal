...
INSERT INTO public.users (id, organization_id, email, role, status, created_at, updated_at)
SELECT id, organization_id, email, role, COALESCE(status, 'active'), created_at, updated_at
FROM public.profiles_legacy
ON CONFLICT (id) DO NOTHING;

INSERT INTO public.profiles (user_id, name, avatar_url, notification_preferences, presence_status, last_seen_at, created_at, updated_at)
SELECT id, name, avatar_url, COALESCE(notification_preferences, '{}'), COALESCE(presence_status, 'offline'), last_seen_at, created_at, updated_at
FROM public.profiles_legacy
...
RETURNS BOOLEAN AS $$
  SELECT EXISTS (
    ...
    FROM public.users u
    JOIN organizations o ON o.id = u.organization_id
    WHERE u.id = auth.uid() AND o.type = 'kre8ivtech'
  )
$$ LANGUAGE sql SECURITY DEFINER STABLE;
  ...
  USING (
    ...
    OR EXISTS (
      ...
      WHERE u.id = profiles.user_id
        AND (u.organization_id = (SELECT organization_id FROM public.users WHERE id = auth.uid()) OR is_super_admin())
    )
  );
...
CREATE POLICY "Super admins and staff can manage payment terms"
  ...
  USING (
    is_super_admin()
    OR EXISTS (SELECT 1 FROM public.users WHERE id = auth.uid() AND role = 'staff')
  )
  WITH CHECK (
    is_super_admin()
    OR EXISTS (SELECT 1 FROM public.users WHERE id = auth.uid() AND role = 'staff')
...
ALTER TABLE public.audit_logs RENAME COLUMN profile_id TO user_id;
...
ALTER TABLE public.form_submissions RENAME COLUMN profile_id TO user_id;
ALTER TABLE public.form_submissions DROP CONSTRAINT IF EXISTS form_submissions_profile_id_fkey;
ALTER TABLE public.form_submissions ADD CONSTRAINT form_submissions_user_id_fkey
  FOREIGN KEY (user_id) REFERENCES public.users(id) ON DELETE SET NULL;
...
ALTER TABLE public.staff_calendar_integrations RENAME COLUMN profile_id TO user_id;
ALTER TABLE public.staff_calendar_integrations DROP CONSTRAINT IF EXISTS staff_calendar_integrations_profile_id_fkey;
ALTER TABLE public.staff_calendar_integrations ADD CONSTRAINT staff_calendar_integrations_user_id_fkey
  FOREIGN KEY (user_id) REFERENCES public.users(id) ON DELETE CASCADE;

-- office_hours: profile_id -> user_id
ALTER TABLE public.office_hours RENAME COLUMN profile_id TO user_id;
ALTER TABLE public.office_hours DROP CONSTRAINT IF EXISTS office_hours_profile_id_fkey;
ALTER TABLE public.office_hours ADD CONSTRAINT office_hours_user_id_fkey
  FOREIGN KEY (user_id) REFERENCES public.users(id) ON DELETE CASCADE;
...
ALTER TABLE public.chat_sessions ADD CONSTRAINT chat_sessions_visitor_id_fkey
  FOREIGN KEY (visitor_id) REFERENCES public.users(id);
ALTER TABLE public.chat_sessions DROP CONSTRAINT IF EXISTS chat_sessions_agent_id_fkey;
ALTER TABLE public.chat_sessions ADD CONSTRAINT chat_sessions_agent_id_fkey
...
ALTER TABLE public.ticket_comments ADD CONSTRAINT ticket_comments_author_id_fkey
...
ALTER TABLE public.vault_items ADD CONSTRAINT vault_items_created_by_fkey
  FOREIGN KEY (created_by) REFERENCES public.users(id) ON DELETE CASCADE;

-- forms: created_by ref users(id)
ALTER TABLE public.forms DROP CONSTRAINT IF EXISTS forms_created_by_fkey;
ALTER TABLE public.forms ADD CONSTRAINT forms_created_by_fkey
  FOREIGN KEY (created_by) REFERENCES public.users(id) ON DELETE CASCADE;
...
ALTER TABLE public.plan_assignments ADD CONSTRAINT plan_assignments_cancellation_requested_by_fkey FOREIGN KEY (cancellation_requested_by) REFERENCES public.users(id);
ALTER TABLE public.plan_assignments DROP CONSTRAINT IF EXISTS plan_assignments_cancelled_by_fkey;
ALTER TABLE public.plan_assignments ADD CONSTRAINT plan_assignments_cancelled_by_fkey FOREIGN KEY (cancelled_by) REFERENCES public.users(id);
...
ALTER TABLE public.billing_disputes ADD CONSTRAINT billing_disputes_submitted_by_fkey FOREIGN KEY (submitted_by) REFERENCES public.users(id);
...
ALTER TABLE public.billing_disputes ADD CONSTRAINT billing_disputes_resolved_by_fkey FOREIGN KEY (resolved_by) REFERENCES public.users(id);
ALTER TABLE public.plan_renewal_notifications DROP CONSTRAINT IF EXISTS plan_renewal_notifications_acknowledged_by_fkey;
ALTER TABLE public.plan_renewal_notifications ADD CONSTRAINT plan_renewal_notifications_acknowledged_by_fkey FOREIGN KEY (acknowledged_by) REFERENCES public.users(id);
ALTER TABLE public.overage_acceptances DROP CONSTRAINT IF EXISTS overage_acceptances_requested_by_fkey;
ALTER TABLE public.overage_acceptances ADD CONSTRAINT overage_acceptances_requested_by_fkey FOREIGN KEY (requested_by) REFERENCES public.users(id);
ALTER TABLE public.overage_acceptances DROP CONSTRAINT IF EXISTS overage_acceptances_accepted_by_fkey;
ALTER TABLE public.overage_acceptances ADD CONSTRAINT overage_acceptances_accepted_by_fkey FOREIGN KEY (accepted_by) REFERENCES public.users(id);
ALTER TABLE public.overage_acceptances DROP CONSTRAINT IF EXISTS overage_acceptances_invoice_approved_by_fkey;
ALTER TABLE public.overage_acceptances ADD CONSTRAINT overage_acceptances_invoice_approved_by_fkey FOREIGN KEY (invoice_approved_by) REFERENCES public.users(id);
...
CREATE POLICY "Users can view active forms for their org or system"
  ...
  USING (
    (status = 'active' AND (organization_id IS NULL OR organization_id = (SELECT organization_id FROM public.users WHERE id = auth.uid())))
    OR EXISTS (SELECT 1 FROM public.users WHERE id = auth.uid() AND role IN ('super_admin', 'staff'))
  );
DROP POLICY IF EXISTS "Staff can manage forms" ON public.forms;
CREATE POLICY "Staff can manage forms"
  ON public.forms FOR ALL
  USING (EXISTS (SELECT 1 FROM public.users WHERE id = auth.uid() AND role IN ('super_admin', 'staff')))
  WITH CHECK (EXISTS (SELECT 1 FROM public.users WHERE id = auth.uid() AND role IN ('super_admin', 'staff')));
...
CREATE POLICY "Staff can view form submissions"
  ...
  USING (EXISTS (SELECT 1 FROM public.users p WHERE p.id = auth.uid() AND p.role IN ('super_admin', 'staff')));
...
CREATE POLICY "Users can submit to active forms"
  ...
  WITH CHECK (
    ...
    AND EXISTS (
      ...
      AND (f.organization_id IS NULL OR f.organization_id = (SELECT organization_id FROM public.users WHERE id = auth.uid()))
    )
  );
...
CREATE POLICY "Staff can view own calendar integrations"
  ...
  USING (user_id = auth.uid() OR EXISTS (SELECT 1 FROM public.users WHERE id = auth.uid() AND role IN ('staff', 'super_admin')));
...
CREATE POLICY "Partners can view client organizations"
  ON organizations FOR SELECT
  USING (parent_org_id = (SELECT organization_id FROM public.users WHERE id = auth.uid()));
...
CREATE POLICY "Users can view relevant comments"
  ...
  USING (
    EXISTS (
      ...
      AND (
        EXISTS (SELECT 1 FROM public.users p WHERE p.id = auth.uid() AND p.role IN ('staff', 'super_admin'))
        OR t.organization_id IN (
          SELECT id FROM organizations
          WHERE id IN (SELECT organization_id FROM public.users WHERE id = auth.uid())
          OR parent_org_id IN (SELECT organization_id FROM public.users WHERE id = auth.uid())
        )
        OR t.organization_id IN (SELECT organization_id FROM public.users WHERE id = auth.uid())
      )
    )
    AND (
      is_internal = FALSE
      OR EXISTS (SELECT 1 FROM public.users p WHERE p.id = auth.uid() AND p.role IN ('staff', 'super_admin', 'partner', 'partner_staff'))
    )
  );
...
CREATE POLICY "Staff can manage all tickets"
  ON tickets FOR ALL
  USING (EXISTS (SELECT 1 FROM public.users WHERE id = auth.uid() AND role IN ('staff', 'super_admin')))
  WITH CHECK (EXISTS (SELECT 1 FROM public.users WHERE id = auth.uid() AND role IN ('staff', 'super_admin')));
...
CREATE POLICY "Partners can manage their and client tickets"
  ...
  USING (
    ...
    OR organization_id IN (SELECT id FROM organizations WHERE parent_org_id IN (SELECT organization_id FROM public.users WHERE id = auth.uid()))
  )
  WITH CHECK (
    organization_id IN (SELECT organization_id FROM public.users WHERE id = auth.uid())
    OR organization_id IN (SELECT id FROM organizations WHERE parent_org_id IN (SELECT organization_id FROM public.users WHERE id = auth.uid()))
...
CREATE POLICY "Partner categories are viewable by partners"
  ...
  USING ((SELECT role FROM public.users WHERE id = auth.uid()) IN ('super_admin', 'staff', 'partner'));
...
CREATE POLICY "Internal categories are viewable by staff"
  ON kb_categories FOR SELECT
  USING ((SELECT role FROM public.users WHERE id = auth.uid()) IN ('super_admin', 'staff'));
...
CREATE OR REPLACE FUNCTION is_admin_or_staff()
RETURNS BOOLEAN AS $$
  SELECT EXISTS (SELECT 1 FROM public.users WHERE id = auth.uid() AND role IN ('super_admin', 'staff'))
$$ LANGUAGE sql SECURITY DEFINER STABLE;
  ...
  USING (
    EXISTS (
      ...
      AND (
        ...
        OR p.organization_id = (SELECT organization_id FROM public.users WHERE id = auth.uid())
      )
    )
  );
...
CREATE POLICY "Users can view their renewal notifications"
  ...
  USING (
    EXISTS (
      ...
      AND pa.organization_id = (SELECT organization_id FROM public.users WHERE id = auth.uid())
    )
  );
