...
-- Allows admins, account managers, and project managers to create platform-wide,
-- client-specific, and staff-specific notifications
...
-- Create notification_reads table to track read/dismissed status
...
CREATE INDEX idx_notifications_is_active ON notifications(is_active) WHERE is_active = TRUE;
CREATE INDEX idx_notifications_created_by ON notifications(created_by);
CREATE INDEX idx_notifications_target_orgs ON notifications USING GIN(target_organization_ids);
...
-- Policy: Staff and account managers can view their created notifications
...
CREATE POLICY "Users can view notifications targeted to them"
  ...
  USING (
    ...
    AND (
      -- Platform-wide notifications to all
      (type = 'platform_wide' AND target_audience = 'all')
      
      -- Platform-wide notifications to clients
      OR (
        type = 'platform_wide' 
        AND target_audience = 'clients'
        AND EXISTS (
          SELECT 1 FROM users
          WHERE users.id = auth.uid()
          AND users.role IN ('client', 'partner')
        )
      )
      
      -- Platform-wide notifications to staff
      OR (
        type = 'platform_wide' 
        AND target_audience = 'staff'
        AND EXISTS (
          SELECT 1 FROM users
          WHERE users.id = auth.uid()
      ...
      -- Client-specific notifications (targeted to specific organizations)
      OR (
        type = 'client_specific'
        AND EXISTS (
          SELECT 1 FROM users
          WHERE users.id = auth.uid()
          AND users.organization_id = ANY(target_organization_ids)
        )
      )
      
      -- Staff-specific notifications (targeted to specific organizations)
      OR (
        type = 'staff_specific'
        AND EXISTS (
          SELECT 1 FROM users
          ...
          AND users.role IN ('staff', 'super_admin', 'partner_staff')
          AND (
            target_organization_ids IS NULL
            OR users.organization_id = ANY(target_organization_ids)
          )
        )
      )
...
-- Policy: Admin can create any notification
...
-- Policy: Account managers can create notifications for staff and admin
CREATE POLICY "Account managers can create staff notifications"
  ...
  WITH CHECK (
    EXISTS (
      SELECT 1 FROM users
      WHERE users.id = auth.uid()
      AND users.is_account_manager = TRUE
      AND users.role IN ('staff', 'partner')
    )
    AND (
      type IN ('platform_wide', 'staff_specific')
      AND target_audience IN ('staff', 'specific_users')
    )
  );

-- Policy: Project managers (staff with assignments) can create notifications for assigned clients and staff
CREATE POLICY "Project managers can create client notifications"
  ...
  WITH CHECK (
    EXISTS (
      SELECT 1 FROM users
      WHERE users.id = auth.uid()
      AND users.role IN ('staff', 'partner_staff')
    )
    AND (
      -- Can create client-specific notifications
      type = 'client_specific'
      -- Can create staff notifications
      OR (type = 'staff_specific' AND target_audience IN ('staff', 'specific_users'))
    )
  );

-- Policy: Admin can update any notification
...
-- Policy: Creators can update their own notifications
...
-- Policy: Creators can delete their own notifications
...
-- Policy: Users can update their own read records
CREATE POLICY "Users can update own notification reads"
  ON notification_reads FOR UPDATE
  USING (user_id = auth.uid());

-- Policy: Admin can view all notification reads
...
-- Create a view for users to easily see their unread notifications
...
SELECT 
  ...
  END AS is_read,
  CASE 
    WHEN nr.dismissed_at IS NOT NULL THEN TRUE
    ELSE FALSE
  END AS is_dismissed,
...
BEGIN
  ...
  WHERE n.is_active = TRUE
    ...
    AND nr.id IS NULL
    AND (
      -- Platform-wide to all
      (n.type = 'platform_wide' AND n.target_audience = 'all')
      
      -- Platform-wide to clients
      OR (
        n.type = 'platform_wide' 
        AND n.target_audience = 'clients'
        AND EXISTS (
          SELECT 1 FROM users
          WHERE users.id = auth.uid()
          AND users.role IN ('client', 'partner')
      ...
      -- Platform-wide to staff
      OR (
        n.type = 'platform_wide' 
        AND n.target_audience = 'staff'
        AND EXISTS (
          SELECT 1 FROM users
          WHERE users.id = auth.uid()
          AND users.role IN ('staff', 'super_admin', 'partner_staff')
      ...
      -- Client-specific
      OR (
        n.type = 'client_specific'
        AND EXISTS (
          SELECT 1 FROM users
          WHERE users.id = auth.uid()
          AND users.organization_id = ANY(n.target_organization_ids)
        )
      )
      
      -- Staff-specific
      OR (
        n.type = 'staff_specific'
        AND EXISTS (
          SELECT 1 FROM users
          WHERE users.id = auth.uid()
          AND users.role IN ('staff', 'super_admin', 'partner_staff')
          AND (
            n.target_organization_ids IS NULL
            OR users.organization_id = ANY(n.target_organization_ids)
          )
        )
      )
      ...
      -- Specific users
      OR (
        n.target_audience = 'specific_users'
        AND auth.uid() = ANY(n.target_user_ids)
      )
    );
...
COMMENT ON TABLE notifications IS 'System-wide notification announcements with role-based targeting';
COMMENT ON TABLE notification_reads IS 'Tracks which users have read or dismissed notifications';
COMMENT ON FUNCTION mark_notification_read IS 'Marks a notification as read for the current user';
COMMENT ON FUNCTION dismiss_notification IS 'Dismisses a notification for the current user';
